...
public class HomeServlet extends HttpServlet {
	...
	//	response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		out.println("<html> <head> <title> Reimbursement Details </title> </head> <body>"
				+ "<a href='createreimburse.html'>Create a Reimbursement Request </a> <br>");
		...
		for (Reimbursement reimbursement : reimburseList) {
			...
			if (isBasakManager == 1) {
                out.println("<td> <a href='ApproveServlet?yes=yes&id=" +reimbursement.getId()
                +"'> Approve </a> ||");
                out.println("<a href='ApproveServlet?yes=no&id=" +reimbursement.getId()
                +"'> Deny </a> </td>");
            }
            out.println("</tr>");
        }
        ...
        if (role.equals(Role.EMPLOYEE)) {
        out.println("<a href='createreimburse.html'> Create Reimbursement Request");
        }
	}
